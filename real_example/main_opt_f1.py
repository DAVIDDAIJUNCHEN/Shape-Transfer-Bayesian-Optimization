        ...
        for i in range(NUM_EXP):
            ...
            if not NOISY:
                if Found:
                    ...
                    step = 0
                    for res, hyper in zip(Bo_test.Y, Bo_test.X):
                        step += 1
                        print(str(step) + "-th responses: ", res)
                        print(str(step) + "-th parameter: ", hyper)
                        print("\n")

                    print("Step " + str(queries) + ': best found was ' + str(Bo_test.Y.max()), sep=',')
            else:
