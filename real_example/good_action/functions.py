...
class Ackley_10:
    ...
    def __call__(self,X):
        ...
        for _ in range(X.shape[0]):
            ...
            _out = -20.0*math.exp(-0.2*math.sqrt(firstSum/10)) - math.exp(secondSum*0.1) + 20 + np.exp(1)
            out.append(-1*_out)  # negative 
...
class Levy_6:
    def __init__(self, noisy=False):
        self.dim=6
        self.bounds=np.array([[-10, 10]] * self.dim)
        self.noisy=noisy
        self.noise_std = 0.05
        self.max = 40.82
    ...
    def __call__(self,X):
        ...
        for _ in range(X.shape[0]):
            ...
            for c in X[0][:5]:
                w_i = 1 + (c-1)/4
                firstSum += (1+10*(math.sin(math.pi*w_i + 1))**2)*(w_i-1)**2
            
            w1 = 1 + (X[0][0] -1)/4
            w6 = 1 + (X[0][5] -1)/4
            _out = (math.sin(math.pi*w1))**2 + firstSum + (1+(math.sin(2*math.pi*w6))**2)*(w6-1)**2
            out.append(-1*_out)  # negative 
