...
# 
# This code implements the Diff-GP 
...
class DiffGP(Model):
    """Difference Gaussian Process (DiffGP)
    
    Transfer Learning model based on [Shilton et al: Regret Bounds for Transfer Learning in Bayesian Optimization](
    https://proceedings.mlr.press/v54/shilton17a.html).
    The Diff-GP method adjusts the target domain prior based on differences between source and target domains to 
    improve prediction performance and compute transfer posterior in Bayesian optimization.
    """
    ...
    def __init__(self, n_features: int, within_model_normalie: bool = True):
        ...
        # GP on target and adjusted source data
        self.final_gp = GPBO(
            RBF(self.n_features, ARD=True),
            noise_variance=0.0, # origin: 0.1
            normalize=False,#self._within_model_normalize,            
    ...
    def meta_fit(
        ...
        self.adjusted_source_datasets = source_datasets
    ...
    def fit(self, data: TaskData, optimize: bool=False):
        """Build target gp based on adjusted source data and target data"""
        if not self.source_gps:
            raise ValueError(
        ...
        for i, (source_id, source_data) in enumerate(self.adjusted_source_datasets.items()):
            X_i = source_data.X
            Y_i_adjusted = self.predict_posterior_mean(InputData(X_i))
            X_all = np.vstack((X_all, X_i))
            Y_all = np.vstack((Y_all, Y_i_adjusted))
        
        self.final_gp.fit(TaskData(X_all, Y_all), optimize)
    ...
    def predict(
        ...
        # returned mu and var of final_gp
        mu, var = self.final_gp.predict(
    ...
    def predict_posterior_mean(self, data: InputData, idx: int = None) -> np.ndarray:
        ...
        """
        if self.already_fit:
            return self.final_gp.predict_posterior_mean(data)
    ...
    def predict_posterior_covariance(self, x1: InputData, x2: InputData) -> np.ndarray:
        ...
        """
        if self.already_fit:
            return self.final_gp.predict_posterior_covariance(x1, x2)
        else:
