...
if [ $stage -eq 0 ] || [ $stage -eq 6 ]; then
    ...
    for i in $(seq 1 $num_rep); do
        echo "Running $i-th simulation"
        mkdir -p $path_data/Double2Double_5sample_1bad_prior_maxminLHS/$i
        out_dir=$path_data/Double2Double_5sample_1bad_prior_maxminLHS/$i
...
if [ $stage -eq 0 ] || [ $stage -eq 9 ]; then
    ...
    for i in $(seq 1 $num_rep); do
        echo "Running $i-th simulation"
        mkdir -p $path_data/2D_Triple2Triple_5sample_2bad_prior_scaleTheta_maxminLHS/$i
        out_dir=$path_data/2D_Triple2Triple_5sample_2bad_prior_scaleTheta_maxminLHS/$i
